...
namespace OpenDebugAD7
{
    ...
    internal class DebugEventLogger
    {
        ...
        private Action<DebugEvent> _sendToRawOutput;
        ...
        /// <param name="rawOutputCallback">Callback to manage logging raw output to avoid recursive logging.</param>
        public DebugEventLogger(Action<DebugEvent> outputCallback, Action<DebugEvent> rawOutputCallback)
        {
            Debug.Assert(outputCallback != null, "Trying to create a logger with no callback!");
            _sendToOutput = outputCallback;
            _sendToRawOutput = rawOutputCallback;
        ...
        /// <summary>
        /// Sends the message line to the raw output callback if the given category has logging enabled.
        /// </summary>
        /// <param name="category">The category of debug event logging.</param>
        /// <param name="message">The message to log.</param>
        /// <param name="data">Logging data to send alongside the message, if any.</param>
        public void WriteLineRaw(LoggingCategory category, string message, Dictionary<string, object> data = null)
        {
            WriteRaw(category, message + Environment.NewLine, data);
        }
